-- all credit goes to https://github.com/msva/lua-htmlparser/tree/master
...
local line = debug and
                 function(lvl) return debug.getinfo(lvl or 2).currentline end or
                 noop
...
function Set:new(values)
    ...
    if type(values) == "table" then
        if not isSet and #values > 0 then
            for _, v in ipairs(values) do instance[v] = true end
        else
            for k in pairs(values) do instance[k] = true end
        end
...
function Set:tolist()
    local res = {}
    for k in pairs(self) do table.insert(res, k) end
    return res
end
...
function ElementNode:settext(c) self.root._text = c end

function ElementNode:textonly() return (self:gettext():gsub("<[^>]*>", "")) end
...
function ElementNode:close(closestart, closeend)
    ...
    while true do
        ...
        if self.id then insert(node.deeperids, self.id, self) end
        ...
        repeat
            ...
            while true do
                ...
                repeat
                    ...
                    if ")" == switch then filter = nil end
                    if "[" == stype and "" ~= quote then
                        local value
                        start, pos, value =
                            string.find(part, "(%b" .. quote .. quote .. ")]",
                                        pos)
                        name = name .. eq .. value
                    end
...
local function parse(text, limit) -- {{{
    ...
    if not opts.keep_danger_placeholders then -- {{{ little speedup by cost of potential parsing breakages
        ...
        "(>)", function(...) return g(2, ...) end):gsub("(" ..
                                                            (tpr["<"] or
                                                                "__FAILED__") .. -- Here we search for "<", we escaped in previous gsub (and don't break things if we have no escaping replacement)
                                                            ")(" ..
                                                            (opts.tpl_marker_pattern or
    ...
    while true do -- MainLoop {{{
        ...
        while true do -- TagLoop {{{
            dbg("[TagLoop]:#LINE# tag.name=%s, tagloop=%s", str(tag.name),
                str(tagloop))
        ...
        while true do -- TagCloseLoop {{{
            ...
            closestart, closeend, closing, closename = root._text:find(
                                                           "[^<]*<(/?)([%w-]+)",
                                                           closeend)
            dbg(
